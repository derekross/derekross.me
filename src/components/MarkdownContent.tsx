...
export function MarkdownContent({
  ...
  const processedContent = useMemo(() => {
    ...
    // Protect existing code blocks from processing
    ...
    // Process Nostr references - link to njump.me
    text = text.replace(/nostr:(npub1|note1|nprofile1|nevent1|naddr1)([023456789acdefghjklmnpqrstuvwxyz]+)/g, (_match, prefix, data) => {
      const nostrId = `${prefix}${data}`;
      return `[nostr:${nostrId}](https://njump.me/${nostrId})`;
    });

    // Process hashtags - link to njump.me (strip # from URL path)
    text = text.replace(/(^|\s)(#(\w+))/g, '$1[$2](https://njump.me/t/$3)');
      ...
      <ReactMarkdown
        components={{
          ...
          a: ({ href, children, ...props }) => {
            // Handle Nostr npub links - render as NostrMention
            if (href?.startsWith('https://njump.me/npub1')) {
              const nip19Id = href.replace('https://njump.me/', '');
              try {
